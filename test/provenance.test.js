...
test('keeps attribution and provenance on the required surfaces', async () => {
  const [html, notice, provenance, browserAnalytics, worker] = await Promise.all([
  ...
  for (const surface of [html, provenance, browserAnalytics, worker]) {
    assert.match(surface, new RegExp(PROVENANCE_ID.replaceAll('.', '\\.')));
  }
  for (const surface of [html, notice, provenance, worker]) {
